...
export interface ScrollMorphHeroData {
  introHeading?: string;
...
export default function IntroAnimation({ data }: { data?: ScrollMorphHeroData }) {
    ...
    const displayImages = useMemo(() => {
        if (data?.images && data.images.length > 0) {
            // Repeat images if fewer than TOTAL_IMAGES to fill the slots
            const imgs = data.images.map(img => img.url);
            while (imgs.length < TOTAL_IMAGES) {
                imgs.push(...imgs);
            }
            return imgs.slice(0, TOTAL_IMAGES);
        }
    ...
    // --- Container Size ---
    useEffect(() => {
        // Use the sticky container for sizing references
    ...
    // --- Native Scroll Logic with useScroll ---
    ...
    // Happens in the first 20% of the scroll container
    ...
    useEffect(() => {
        // Attach listener to window for smoother parallax during scroll
    ...
    return (
        <div ref={containerRef} className="relative w-full h-[300vh] bg-[#FAFAFA]">
            {/* Sticky Viewport */}
            <div ref={stickyRef} className="sticky top-0 h-screen w-full overflow-hidden flex flex-col items-center justify-center perspective-1000">
                ...
                <div className="absolute z-0 flex flex-col items-center justify-center text-center pointer-events-none top-1/2 -translate-y-1/2">
                    ...
                    >
                        {data?.introHeading || "The future is built on AI."}
                    </motion.h1>
                    <motion.p
                        initial={{ opacity: 0 }}
                    ...
                    >
                        {data?.introSubheading || "SCROLL TO EXPLORE"}
                    </motion.p>
                </div>
                ...
                >
                    <h2 className="text-3xl md:text-5xl font-semibold text-gray-900 tracking-tight mb-4">
                        {data?.sectionTitle || "Explore Our Vision"}
                    </h2>
                    <p className="text-sm md:text-base text-gray-600 max-w-lg leading-relaxed">
                        {data?.sectionDescription || "Discover a world where technology meets creativity. Scroll through our curated collection of innovations designed to shape the future."}
                    </p>
                </motion.div>
